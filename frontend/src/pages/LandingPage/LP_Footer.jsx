...
export default function LP_Footer() {
  ...
  const appLinks = t("lp.footer.getApp.links", { returnObjects: true });
      ...
      <Box sx={{ maxWidth: "1400px", mx: "auto" }}>
        <Grid container spacing={8}>
          ...
          <Grid item xs={6} md={2}>
            ...
            {productLinks.map((link) => <FooterLink key={link}>{link}</FooterLink>)}
          </Grid>

          {/* --- COLUMN 3: COMPANY --- */}
          <Grid item xs={6} md={2}>
            <FooterHeader>{t("lp.footer.company.heading")}</FooterHeader>
            {companyLinks.map((link) => <FooterLink key={link}>{link}</FooterLink>)}
          </Grid>

          {/* --- COLUMN 4: LEGAL --- */}
          <Grid item xs={6} md={2}>
            <FooterHeader>{t("lp.footer.legal.heading")}</FooterHeader>
            {legalLinks.map((link) => <FooterLink key={link}>{link}</FooterLink>)}
          </Grid>

          {/* --- COLUMN 5: DOWNLOAD --- */}
          <Grid item xs={6} md={2}>
            <FooterHeader>{t("lp.footer.getApp.heading")}</FooterHeader>
            {appLinks.map((link) => <FooterLink key={link}>{link}</FooterLink>)}
          </Grid>
        </Grid>
